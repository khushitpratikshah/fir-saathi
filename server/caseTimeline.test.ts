...
describe("case-status timeline", () => {
  it("uses the newest real audit event for each completed lifecycle step", () => {
    ...
    expect(timeline.stages.map((stage) => stage.label)).toEqual(["Intake created", "Transcript captured", "Evidence fingerprint checked", "Draft prepared", "Citizen confirmed", "Human verified"]);
    expect(timeline.stages[3].event?.createdAt.toISOString()).toBe("2026-08-01T12:00:00.000Z");
    expect(timeline.stages[5].event).toBeUndefined();
  });
  ...
  it("shows the returned clarification loop before a future human verification", () => {
    ...
    expect(timeline.stages.map((stage) => stage.label)).toEqual(["Intake created", "Transcript captured", "Evidence fingerprint checked", "Draft prepared", "Citizen confirmed", "Clarification requested", "Citizen clarified", "Human verified"]);
