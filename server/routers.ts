...
import { isSyntheticDemoReviewer, SYNTHETIC_DEMO_PUBLIC_ID } from "./demoMode";
...
export const appRouter = router({
  ...
  complaints: router({
    list: constableProcedure.query(async ({ ctx }) => { try { return await listComplaints(isSyntheticDemoReviewer(ctx.user) ? SYNTHETIC_DEMO_PUBLIC_ID : undefined); } catch (error) { return databaseError(error); } }),
    ...
    review: constableProcedure.input(z.object({ publicId: publicIdSchema })).query(async ({ input, ctx }) => {
      try {
        if (isSyntheticDemoReviewer(ctx.user) && input.publicId !== SYNTHETIC_DEMO_PUBLIC_ID) throw new TRPCError({ code: "NOT_FOUND", message: "This demo workspace contains one synthetic review case." });
        const detail = await getComplaintDetail(input.publicId);
        if (!detail) throw new TRPCError({ code: "NOT_FOUND", message: "This review record was not found." });
        return detail;
      } catch (error) { if (error instanceof TRPCError) throw error; return databaseError(error); }
    }),
    reviewerTranslation: constableProcedure.input(z.object({ publicId: publicIdSchema })).mutation(async ({ input, ctx }) => {
      try {
        if (isSyntheticDemoReviewer(ctx.user) && input.publicId !== SYNTHETIC_DEMO_PUBLIC_ID) throw new TRPCError({ code: "NOT_FOUND", message: "This demo workspace contains one synthetic review case." });
        const detail = await getComplaintDetail(input.publicId);
        if (!detail) throw new TRPCError({ code: "NOT_FOUND", message: "This review record was not found." });
    ...
    draft: constableProcedure.input(z.object({ publicId: publicIdSchema })).mutation(async ({ input, ctx }) => { try { if (isSyntheticDemoReviewer(ctx.user) && input.publicId === SYNTHETIC_DEMO_PUBLIC_ID) { const detail = await getComplaintDetail(input.publicId); if (!detail) throw new TRPCError({ code: "NOT_FOUND", message: "This review record was not found." }); return detail.complaint.draftJson; } return await draftComplaint(input.publicId); } catch (error) { if (error instanceof TRPCError) throw error; return databaseError(error); } }),
    ...
    correctField: constableProcedure.input(z.object({ publicId: publicIdSchema, fieldKey: z.string().trim().min(1).max(80), label: z.string().trim().min(1).max(160), value: z.string().trim().min(1).max(5_000), reason: z.string().trim().min(4).max(2_000) })).mutation(async ({ input, ctx }) => { try { if (isSyntheticDemoReviewer(ctx.user) && input.publicId === SYNTHETIC_DEMO_PUBLIC_ID) return { success: true }; await correctComplaintField({ ...input, actorLabel: constableLabel(ctx.user) }); return { success: true }; } catch (error) { return databaseError(error); } }),
    returnForCorrection: constableProcedure.input(z.object({ publicId: publicIdSchema, reason: z.string().trim().min(4).max(2_000) })).mutation(async ({ input, ctx }) => { try { if (isSyntheticDemoReviewer(ctx.user) && input.publicId === SYNTHETIC_DEMO_PUBLIC_ID) return { success: true }; await returnComplaint({ ...input, actorLabel: constableLabel(ctx.user) }); return { success: true }; } catch (error) { return databaseError(error); } }),
    verify: constableProcedure.input(z.object({ publicId: publicIdSchema })).mutation(async ({ input, ctx }) => { try { if (isSyntheticDemoReviewer(ctx.user) && input.publicId === SYNTHETIC_DEMO_PUBLIC_ID) return { success: true }; await verifyComplaint({ ...input, actorLabel: constableLabel(ctx.user) }); return { success: true }; } catch (error) { return databaseError(error); } }),
  }),
  ...
  evidence: router({
    ...
    verifyHash: constableProcedure.input(z.object({ publicId: publicIdSchema, evidenceId: z.string().uuid() })).mutation(async ({ input, ctx }) => { try { if (isSyntheticDemoReviewer(ctx.user) && input.publicId === SYNTHETIC_DEMO_PUBLIC_ID) return { tamperStatus: "unavailable" as const }; return await verifyEvidenceHash({ ...input, actorLabel: constableLabel(ctx.user) }); } catch (error) { return databaseError(error); } }),
  }),
});
