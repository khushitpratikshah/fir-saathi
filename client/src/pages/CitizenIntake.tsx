...
import { JUDGE_SCENARIOS, type JudgeScenario } from "@/lib/judgeScenarios";
import VerifierTrace from "@/components/VerifierTrace";
...
export default function CitizenIntake() {
  ...
  const [judgeScenario, setJudgeScenario] = useState<JudgeScenario | null>(null);
  ...
  const loadJudgeScenario = (scenario: JudgeScenario) => {
    if (activeVoiceAttemptRef.current) cancelVoicePreparation();
    setJudgeScenario(scenario);
    setLanguage(scenario.language);
    setMode("text");
    setSourceText(scenario.transcript);
    setConsent(true);
    setRecordingState("idle");
    setAudioBlob(null);
    setAudioPreviewUrl(null);
    setActiveStep(1);
    toast.success("Synthetic scenario loaded", { description: "No microphone or network call was used. The source-preserving trace is ready below." });
  };
  const goForward = () => {
