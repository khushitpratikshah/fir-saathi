...
import { Check, ChevronRight, CircleAlert, FileText, Info, Keyboard, Loader2, MapPin, Mic, Phone, Play, RotateCcw, ShieldCheck, Square, Volume2 } from "lucide-react";
...
type CitizenContextInput = { incident_when: string; incident_where: string; people_or_vehicle: string; property_or_loss: string; injury_or_safety: string; follow_up_contact: string };
const emptyContext: CitizenContextInput = { incident_when: "", incident_where: "", people_or_vehicle: "", property_or_loss: "", injury_or_safety: "", follow_up_contact: "" };
const contextFields: Array<{ key: keyof CitizenContextInput; label: string; placeholder: string }> = [
  { key: "incident_when", label: "When did it happen?", placeholder: "Date, time, or an approximate time" },
  { key: "incident_where", label: "Where did it happen?", placeholder: "Area, street, landmark, or route" },
  { key: "people_or_vehicle", label: "People or vehicle details", placeholder: "Only details you want to share" },
  { key: "property_or_loss", label: "Property or loss details", placeholder: "Item, document, amount, or identifier" },
  { key: "injury_or_safety", label: "Injury or safety detail", placeholder: "Any injury, threat, or current concern" },
  { key: "follow_up_contact", label: "Safe follow-up contact or time", placeholder: "Optional phone, email, or preferred time" },
];
...
export default function CitizenIntake() {
  ...
  const [urgentSafety, setUrgentSafety] = useState(false);
  ...
  const submitVoice = async () => {
    ...
    try {
      ...
      createVoiceComplaint.mutate({ language, mimeType, rawAudioBase64: toBase64(rawBytes), encryptedAudioBase64: toBase64(encryptedBytes), ivBase64: toBase64(iv), ciphertextSha256: Array.from(digest).map((byte) => byte.toString(16).padStart(2, "0")).join(""), context }, {
        onSuccess: ({ publicId }) => {
