    ...
    <FirSaathiShell compact>
      <main className="page-grid min-h-[calc(100vh-144px)] py-8 sm:py-12">
        <div className="mx-auto max-w-4xl px-5 sm:px-8">
          ...
          <section className="workspace-panel mt-5 overflow-hidden rounded-[1.4rem] border border-[#102643]/10 bg-[#fbfaf6]/95 shadow-[var(--panel-shadow)]">
            <div className="border-b border-[#102643]/10 bg-[linear-gradient(112deg,rgba(255,255,255,.94),rgba(252,233,223,.42))] px-5 py-6 sm:px-7">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div><p className="page-eyebrow">Citizen confirmation</p><h1 className="page-title mt-2 text-3xl font-bold">Check what will go to review.</h1><p className="page-lede mt-2 text-sm">Your source statement is shown separately. The structured area is an assistance draft; it does not change what you said.</p></div>
            ...
            <div className="space-y-6 p-5 sm:p-7">
              ...
              <aside className="info-callout rounded-2xl border border-[#c64e19]/20 p-4"><div className="flex gap-3"><FileText className="mt-0.5 h-4 w-4 shrink-0 text-[#c64e19]" /><div><p className="text-sm font-bold text-[#8f360e]">Before you send this to review</p><p className="mt-1 text-xs leading-5 text-[#8f360e]">Your explicit confirmation sends a draft to the constable workspace. It does not register an FIR or decide any legal section.</p></div></div></aside>
              <div className="flex flex-col gap-3 border-t border-[#102643]/10 pt-5 sm:flex-row sm:items-center sm:justify-between"><p className="flex items-start gap-2 text-xs leading-5 text-slate-500"><CircleAlert className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#c64e19]" /> Your confirmation sends a draft to a constable. It does not register an FIR.</p><button type="button" disabled={confirm.isPending || complaint.status === "verified" || (!hasListened && !fallbackAcknowledged)} onClick={() => confirm.mutate({ publicId })} className="action-primary pressable focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#102643] px-4 text-sm font-bold text-white disabled:opacity-50">{confirm.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}{complaint.status === "returned" ? "Send response for review" : "Confirm and send for review"}</button></div>
            </div>
          </section>
        </div>
      </main>
