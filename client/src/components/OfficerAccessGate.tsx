...
export function useOfficerAccess() {
  const auth = useAuth();
  return { ...auth, isOfficer: auth.user?.role === "admin" };
}
...
function PortableSignIn() {
  ...
  return <main className="grid min-h-[calc(100vh-144px)] place-items-center px-5"><section className="max-w-md rounded-2xl border border-[#102643]/10 bg-white p-7 text-center shadow-[0_18px_45px_rgba(12,32,57,.09)]"><span className="mx-auto grid h-11 w-11 place-items-center rounded-full bg-[#fff0e8] text-[#c64e19]"><LockKeyhole className="h-5 w-5" /></span><h1 className="mt-4 text-xl font-bold text-[#102643]">Constable sign-in required</h1><p className="mt-2 text-sm leading-6 text-slate-600">Use your FIR Saathi account to open the review queue. Constable access is assigned in Supabase and enforced by the server.</p><form onSubmit={submit} className="mt-5 space-y-3 text-left">{mode === "sign-up" && <input required value={name} onChange={(event) => setName(event.target.value)} className="focus-ring h-11 w-full rounded-xl border border-[#102643]/15 px-3 text-sm" placeholder="Display name" autoComplete="name" />}<input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} className="focus-ring h-11 w-full rounded-xl border border-[#102643]/15 px-3 text-sm" placeholder="Email address" autoComplete="email" /><input required type="password" minLength={6} value={password} onChange={(event) => setPassword(event.target.value)} className="focus-ring h-11 w-full rounded-xl border border-[#102643]/15 px-3 text-sm" placeholder="Password" autoComplete={mode === "sign-in" ? "current-password" : "new-password"} /><button type="submit" disabled={establish.isPending} className="focus-ring w-full rounded-xl bg-[#102643] px-4 py-3 text-sm font-bold text-white disabled:opacity-60">{establish.isPending ? "Verifying session…" : mode === "sign-in" ? "Sign in to review records" : "Create FIR Saathi account"}</button></form>{notice && <p className="mt-3 text-xs leading-5 text-emerald-800">{notice}</p>}{error && <p className="mt-3 text-xs leading-5 text-[#9b3a0d]">{error}</p>}<button type="button" onClick={() => { setMode((current) => current === "sign-in" ? "sign-up" : "sign-in"); setError(""); setNotice(""); }} className="focus-ring mt-4 text-xs font-bold text-[#102643] underline underline-offset-4">{mode === "sign-in" ? "Need an account? Sign up" : "Already registered? Sign in"}</button></section></main>;
}
