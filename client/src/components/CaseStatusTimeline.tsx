import { CheckCircle2, CircleDotDashed, Clock3, FilePenLine, Fingerprint, LockKeyhole, MessageSquareText, SendHorizonal, ShieldCheck, Undo2 } from "lucide-react";
...
export default function CaseStatusTimeline({ status, audit }: Props) {
  ...
  const integrityEvents = [...audit].filter((event) => ["created", "transcribed", "evidence_checked", "citizen_confirmed", "field_corrected", "verified"].includes(event.eventType)).sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
